...
class NewsCreationPlan extends Indicator {
    ...
    public function getValues() {
        ...
        $query = $this->conn->newStatement("
            ...
            WHERE DATE_FORMAT(date_creation,'%Y-%m')>=:f: AND DATE_FORMAT(date_creation,'%Y-%m')<=:t: AND user_id IN (:uid".implode(':,:uid',array_keys($this->users)).":) AND type=1
    ...
    public function calculate() {
        ...
        $query = $this->db->newStatement("
            SELECT COUNT(category_id) cnt, user_id, DATE_FORMAT(date_creation,'%Y-%m') date FROM log_article
            WHERE DATE_FORMAT(date_creation,'%Y-%m')>=:f: AND DATE_FORMAT(date_creation,'%Y-%m')<=:t: AND user_id IN (:uids:) AND type=1
            GROUP BY user_id, DATE_FORMAT(date_creation,'%Y-%m')
        ");
        $query->setVarChar('f',$this->start);
        $query->setVarChar('t',$this->end);
