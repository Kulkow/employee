...
class TotalSaranskOrdersCreationPlan extends Indicator {
    ...
    public function calculate() {
        ...
        if ($this->start < '2015-11') {
            ...
            $select = "
                ...
                ";
            $from = "
                LEFT OUTER JOIN pm_employee pe ON pe.user_id=op.creator_id AND pe.department_id=5 AND pe.`start`<=DATE_FORMAT(:till:,'%Y-%m-%d') AND (pe.`end`>=DATE_FORMAT(:since:,'%Y-%m-%d') OR pe.`end` IS NULL)
                LEFT OUTER JOIN employee_data ed ON ed.user_id=b.operator_id
                LEFT OUTER JOIN warehouse w ON w.id=ed.point_id
                ";
            $clause = "
                (pe.id AND b.datetime BETWEEN CONCAT(pe.`start`,' 00:00:00') AND CONCAT(IFNULL(pe.`end`,DATE_FORMAT(NOW(),'%Y-%m-%d')),' 23:59:59') OR w.city_id=63)
                ";
            $params = [
