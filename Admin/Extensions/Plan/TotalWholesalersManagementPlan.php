...
class TotalWholesalersManagementPlan extends Indicator {
    ...
    public function getValues() {
        ...
        $query = $this->conn->newStatement("
            ...
            FROM (
                (SELECT op1.manager_id, op1.creator_id, o1.owner_id, op1.status_date, op1.warehouse_id,
                    op1.quantity*(op1.price-op1.purchase_price-op1.transport_charges)*IF(o1.user_id=3199,0.5,1) profit, o1.id
                ...
                (SELECT op2.manager_id manager_id, IF(pctp.manager_id=0,op2.creator_id,pctp.manager_id) creator_id, o2.owner_id, op2.status_date, op2.warehouse_id,
                    IF(pctp.change_type_id IN (11,61),0,pctp.amount) profit, o2.id
            ...
            INNER JOIN `order` o ON o.id=a.id
            INNER JOIN user u2 ON u2.id=o.user_id
            WHERE a.owner_id=:oid: AND DATE_FORMAT(a.status_date,'%Y-%m')>=:f:
            AND DATE_FORMAT(a.status_date,'%Y-%m')<=:t: AND u2.role_id IN (:uf:,:uw:)
        ...
        $query->setInteger('uf', USER_FRANCHISER);
        $query->setInteger('uw', USER_WHOLESALER);
